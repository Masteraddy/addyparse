...
function optionsFromArguments(args) {
  let options = args;
  options = requiredOrFromEnvironment(
    options,
    "cloud_name",
...
function Cloudinary(data) {
  ...
  try {
    const made = mkdirp.sync(__dirname + path.sep + this.TempFolder);
    cloudinary.config({
      cloud_name: options.cloud_name,
      api_key: options.api_key,
      api_secret: options.api_secret,
    });
    console.log("temp folder made ", made);
  } catch (error) {
    console.log(error);
  }
}
...
Cloudinary.prototype.deleteFile = function (filename) {
  ...
  return new Promise(function (resolve, reject) {
    ifWebp(folder, filename).then(function (_filename) {
      filename = _filename;
      fs.unlink(
        __dirname + path.sep + ".." + path.sep + folder + path.sep + filename,
        function (err) {
          if (err) {
            console.log("err", err);
            reject(err);
...
Cloudinary.prototype.getFileLocation = function (config, filename) {
  let { quality_auto } = this.cloudinary_config;
  return (
    "https://res.cloudinary.com/" +
    this.cloudinary_config.cloud_name +
    "/" +
    this.getResourceType(filename) +
    "/upload/" +
    (quality_auto ? "q_auto/" : "") +
...
Cloudinary.prototype.getResourceType = (filename) => {
  ...
  if (/\.(gif|jpg|jpeg|tiff|png)$/i.test(filename)) {
    ...
    /\.(mp4|3gp|3g2|mpeg|mpg|amv|wmv|mov|avi|ogg|flv|webm|mkv)$/i.test(filename)
  ) {
    ResourceType = "video";
  }
  return ResourceType;
